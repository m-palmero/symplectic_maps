...
int maps_details()
{
    ...
    if (boozer_map)
    {
        //booz_map.parameter = 0.6; //Standard
        ...
        if(phase_space_analysis)
        {
            switch (ic_set)
            {
                case 1:
                    ...
                    //change(initial_conditions, 6.6855256904945187e-04, 9.977400780170988e-01);
                    change(initial_conditions, 0.0006685, 0.99774007);
                    asprintf(&ic_label, "ensemble");
                    break;
                case 2:
                    booz_map.parameter = 0.6056; //Local high escape rate
                    //change(initial_conditions, 1e-15,  9.9698287930271623e-01);
